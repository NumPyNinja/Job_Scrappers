...
class Glassdoor:
    ...
    def glassdoor_search_jobs(self,job_name,job_location):
        # pass jobname and location
        try:
            self.driver.find_element_by_id("HeroSearchButton").click()
            self.driver.find_element_by_id("sc.keyword").send_keys(job_name)
            Glassdoor.job_details["Searched Job Title"] = job_name
            messagebox = self.driver.find_element_by_id("sc.location")
            messagebox.send_keys(job_location)
            Glassdoor.job_details["Searched Job Location"] = job_location
    ...
    def glassdoor_links(self, soup, page_count, count):
        ...
        for links in soup.find_all('div', {'class': 'jobHeader'}):
            ...
            for link in links.find_all('a'):
                self.List_of_urls.append(link.get('href'))
