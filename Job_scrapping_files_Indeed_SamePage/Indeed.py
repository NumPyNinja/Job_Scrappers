...
class IndeedMain:
    ...
    def indeed_iterating_job_location(self, job_title,job_location,jp_common):
        try:
            ...
            print("checki inside", job_title)
            key = jp_common.job_categorisation(job_title)
    ...
    def indeed_website_search_24hrs(self, jp_common, job, location):
        ...
        try:
            self.indeed_alert()
            #self.driver.find_element_by_xpath("/html/body/table[1]/tbody/tr/td/div[2]/span[1]/button/span").click()
            #self.driver.find_element_by_xpath("/html/body/table[1]/tbody/tr/td/div[2]/span[1]/ul/li[1]/a/span").click()
            ...
            #self.driver.find_element_by_xpath("//*[@id='filter-dateposted']/button/span").click()
            date_posted_element = WebDriverWait(self.driver, 5).until(EC.element_to_be_clickable((By.XPATH, "//*[@id='filter-dateposted']/button/span")))
            #self.indeed_alert()
            date_posted_element.click()
            #self.indeed_alert()
            #past_24hrs = self.driver.find_element_by_xpath("//*[@id='filter-dateposted-menu']/li/a[1]")
            past_24hrs_element = WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable((By.XPATH, "//*[@id='filter-dateposted-menu']/li/a[1]")))
            #self.indeed_alert()
            ...
            #breakpoint()
            try:
                wrong_search_element = WebDriverWait(self.driver, 5).until(
                    EC.presence_of_element_located((By.XPATH, "//div[@class='bad_query']/h1")))
                print(wrong_search_element.text)
                if wrong_search_element.text.find("did not match any jobs"):
                    print("No jobs found:", job, location)
            except Exception as e:
                print("Unknown exception in indeed_website_search_24hrs wrong search element not found ", e)
            ...
            #logging.info("No jobs found:" + job,+" "+location)
        except Exception as e:
